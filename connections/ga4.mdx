...
description: "CleanClicks sends conversion events to Google Analytics 4 via the Measurement Protocol and can proxy your GA4 client-side scripts through your first-p"
---
...
## Configuration (No OAuth Required)

GA4 uses an API key rather than OAuth. No connection step is needed on the Connections page.
...
## GA4 First-Party Proxy (Clarity+ or Analytics Add-On)
...
### Important: WordPress Sites

If you use the CleanClicks WordPress plugin with GA4, the plugin handles GA4 integration client-side. **Do not** also enable Measurement Protocol for the same site, as this creates duplicate events in GA4. The plugin + proxy combination gives you both ad-blocker bypass and proper attribution without duplication.

**Shopify is the exception.** Because Shopify's pixel runs in a sandbox that can't push to GA4's data layer, Measurement Protocol is the only way to get ecommerce events into GA4. Keep it enabled for Shopify.
