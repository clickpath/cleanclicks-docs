...
Create Inbound Integration Keys in **Configuration** > **Inbound keys** tab. See [Inbound Integration Keys](/configuration/api-keys).
...
Authentication and validation errors return `{ "ok": false, "error": "..." }` — e.g. `Invalid API key` (401), `email or hashed_email required` (400), `Inbound webhook not enabled` (403). Malformed requests (bad content type, oversized body, invalid JSON) return a plain-text error with the matching status.
