...
| Custom Pixel (v2.2.0) | Signal+ | Tracks 7 ecommerce events in the pixel sandbox |
| Server-Side Webhook | Signal+ | Captures orders directly from Shopify servers |
...
## Step 2: Install the Custom Pixel (Signal+)

The Custom Pixel v2.2.0 runs in Shopify's sandboxed iframe and tracks the full ecommerce funnel. When GA4 is configured, it also sends a GA4 purchase event via Measurement Protocol on checkout completion.
...
## Step 3: Server-Side Webhook (Signal+)
...
> **Refunds and cancellations:** to reverse refunded value in your ad platforms, also subscribe Shopify's **Refund created** and **Order cancellation** events to the same webhook URL, then map the `refund` (and `cancellation`) event for each platform in **Configuration → Vendors**. Without that mapping the reversal is recorded but not uploaded, so a refund is never sent as a purchase.
