---
title: "API Keys"
description: "API keys let you send conversion events to CleanClicks from external systems: CRMs, marketing automation platforms, webhook-based integrations, or cus"
---

API keys let you send conversion events to CleanClicks from external systems: CRMs, marketing automation platforms, webhook-based integrations, or custom backends.
...
The full API key is displayed **once**. Copy it immediately and store it securely. After you close the dialog, you'll only see a masked version.

API keys use a `cc_` prefix followed by a random string. Example: `cc_a1b2c3d4e5f6...`

## Using an API Key

Include the API key in the `X-CC-Api-Key` header when sending events to the CleanClicks inbound endpoint:
...
The API Keys tab shows all keys for the selected domain:
...
- **Never put API keys in client-side code.** API keys are for server-to-server communication only. They should never appear in JavaScript, HTML, or any code that runs in a browser.
